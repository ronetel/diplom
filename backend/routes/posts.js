  ...
  try {
    ...
    if (outfit_id) {
      ...
      if (outfitResult.rows[0].owner_id !== userId) {
        return res.status(403).json({ message: 'Outfit does not belong to you' })
      }
    }
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (postResult.rows[0].author_id !== userId && !['moderator', 'admin'].includes(req.user.role)) {
...
router.delete('/:postId/comments/:commentId', auth, async (req, res) => {
  try {
    ...
    // Только автор или модератор/админ может удалять
    if (commentResult.rows[0].author_id !== userId && !['moderator', 'admin'].includes(req.user.role)) {
      return res.status(403).json({ message: 'Not authorized' })
    }
